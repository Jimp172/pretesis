...
//variable que viene de vista para guardar
if(isset($_GET['guardar']))
{
	//variables que vienen por post de la vista
	$parametros = $_POST;
...
if(isset($_GET['validar_reseptor']))
{
	$datos = $controlador->validar_reseptor();
	echo json_encode($datos);
}
...
class usuarioC
{
	...
	function listar_usuarios($parametros)
	{
		...
		foreach ($datos as $key => $value) {
			$t = '';
			if($value['canal_telegram']==1)
			{
				$t = '<span class="right badge badge-danger">Reseptor Telegram</span>';
			}
			$tr.="<tr>
			<td>
			  <button type='button' class='btn btn-sm btn-primary' onclick='limpiar();editar(".$value['id'].")'><i class='icon fa fa-pen'></i></button>
			  <button type='button' class='btn btn-sm btn-danger' onclick='eliminar_registro(".$value['id'].")'><i class='icon fa fa-trash'></i></button>
			</td>
			<td>".$value['nombre']."<br>".$t."</td>
			<td>".$value['ci_ruc']."</td>
			<td>".$value['nick']."</td>
			<td>".$value['pass']."</td>
			<td>".$value['tipo']."</td>
	...
	//funcion para guarda creando un array 
	 //campo : nombre de la tabla en base de datos
	 //datos : valor a colocar en la base de datos
	function guardar($parametros)
	{
		...
		$datos[6]['campo'] = 'id_telegram';
		$datos[6]['dato'] = $parametros['txt_telegram'];		
		$datos[7]['campo'] = 'canal_telegram';
		if(!isset($parametros['cbx_predeterminado']))
		{
			$datos[7]['dato'] = 0;
		}else
		{
		...
		if($parametros['txt_id']!='')
		{
			...
			//funcion que envia a guardar o edita 
	...
	{
		$datos = $this->modelo->validar_reseptor();
		if(count($datos)>0)
		{
			return -1;
