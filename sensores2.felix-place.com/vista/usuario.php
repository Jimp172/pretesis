...
<script type="text/javascript">
  ...
  function validar_reseptor()
  {
     $.ajax({
      ...
      url:   '../controlador/usuarioC.php?validar_reseptor=true',
      type:  'post',
      dataType: 'json',
      success:  function (response) { 
        ...
        if(response==-1)
        {
          $('#cbx_predeterminado').prop('disabled',true);
        }             
      }
    });
  }
  ...
  // funcion para guardar o editar usuario nuevo usuarios nuevo
	function guardar()
	{
		...
		var t = $('#ddl_tipo').val();
		var id = $('#txt_id').val();
    var tel = $('#txt_telegram').val();
    var pre = $('#cbx_predeterminado').prop('checked');
		if(n=='' || c==''|| k=='' || p=='' || d==''|| t=='' || tel=='')
		{
	...
	function editar(id)
	{
      ...
      success:  function (response) { 
        ...
        $('#txt_telegram').val(response[0].tele);   
        $('#cbx_predeterminado').prop('disabled',false);
        if(response[0].canal_telegram=='1')
        {
          $('#cbx_predeterminado').prop('checked',true);  
        }else
        {
          $('#cbx_predeterminado').prop('checked',false);  
        }      
      }
      });
            ...
            <div class="card shadow mb-8">
                ...
                <div class="card-body">
                	<div class="row">
                        ...
                        <div class="col-sm-7">
                          <b>Telegram ID</b>
                          <input type="text" class="form-control form-control-sm" placeholder="Password" name="txt_telegram" id="txt_telegram">                         
                        </div>  
                        <div class="col-sm-5">
                          <b>Predeterminado</b>
                          <label><input type="checkbox" name="cbx_predeterminado" id="cbx_predeterminado"> SI</label>    
                        </div>                        
                				<div class="col-sm-12">
                					<b>Tipo de usuario</b>
